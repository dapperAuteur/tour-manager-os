 ...
 * `sendToInbox(...)` AFTER the user-facing response is rendered (fire-and-forget
 * via Next.js `after()`); never block the user on it. Dependency-free apart
 * from Node's built-in `crypto` module and the runtime `fetch`.
 *
 * Contract: https://inbox.witus.online + witus-inbox/docs/webhook-contract.md
 *
 ...
 *      your "thank you" UI renders (for example, via Next.js `after()`).
 *   3. Log at most `source`, `form_type`, and the HTTP status. Never log the
 *      submission body, the secret, or the signature.
 *
 * NOTE: this is the INBOX sender (inbound user submissions → BAM's triage
 * queue). It is intentionally separate from `lib/sender-outbox.ts`, which is
 * the OUTBOX sender (outbound social posts) and has a different payload shape.
 */
