...
- ✅ One-page tour settlement PDF — `/api/tours/[id]/settlement-pdf` builds a printable settlement covering ticket sales, merch, guarantees, expenses by category, net, configured revenue splits, and routed Stripe transfers. Downloadable from the tour finances page. Built with `pdf-lib` so no new dependencies
...
- ✅ WanderLearn virtual tour per show — a band member pastes a WanderLearn 360 tour embed (or link) on the show detail page; the public event page renders it in a controlled &ldquo;Virtual tour&rdquo; iframe for a virtual / behind-the-scenes experience. Nullable `shows.wanderlearn_url` (migration 094), `POST /api/shows/[id]/wanderlearn` behind tour-member auth. Host is allowlisted to `wanderlearn.witus.online` at write AND render time; we store only the extracted src URL and never inject pasted markup
...
- ✅ Stripe Connect split payments to artist/venue/crew (Phase 24.1) — `stripe_connected_accounts` (per-org Express account) + `tour_revenue_splits` (per-tour basis-point allocations) + `tour_split_transfers` (audit trail of every fan-out). `/admin/stripe-connect` runs Express onboarding; `/tours/[id]/finances/splits` configures the allocation with a green-at-100% gauge. After every ticket sale, the Stripe webhook fans out `stripe.transfers.create` for each split row that carries a `stripe_account_id`, writing a `tour_split_transfers` row per attempt. Idempotency: `(payment_intent, split)` is unique and the Transfer call passes the same string as its `idempotencyKey` so webhook retries land on the same Transfer object. Splits that don't total 100% are skipped (logged) instead of partial-paid; rows without a Stripe account record a `skipped` audit row so the band knows who they still owe off-platform
- ✅ Apple Wallet `.pkpass` ticket delivery — `lib/wallet/apple-pkpass.ts` builds a signed eventTicket pass via `passkit-generator` (primary field: ticket type; secondary: artist + venue; auxiliary: date + doors; back fields: purchaser, address, web-fallback URL; barcode: same JSON the camera scanner reads so a Wallet QR is interchangeable with the in-app QR). `GET /api/tickets/[id]/pkpass?token=<sig>` verifies the HMAC, refuses refunded/void tickets, returns the signed `.pkpass` blob with `application/vnd.apple.pkpass`. Ticket page surfaces an **Add to Apple Wallet** button. Signing requires platform-admin operator setup: see `plans/user-tasks/33-apple-wallet-cert.md` to install the Pass Type ID + WWDR certs (`PKPASS_CERT_PEM`, `PKPASS_KEY_PEM`, `PKPASS_WWDR_PEM`, `PKPASS_PASS_TYPE_ID`, `PKPASS_TEAM_ID`). Until that&rsquo;s done the route returns 503 instead of an unsigned file
- ✅ Google Wallet ticket delivery — `lib/wallet/google-wallet.ts` builds a signed Save-to-Google-Wallet JWT (RS256, no extra deps) with an `eventTicketObject` carrying the artist, venue, date, ticket type, holder name, and the same `{v,id,sig}` QR string the camera scanner reads. `GET /api/tickets/[id]/google-wallet?token=<sig>` verifies the HMAC, refuses refunded/void tickets, and redirects the fan to `pay.google.com/gp/v/save/<JWT>` for the Google Wallet add prompt. Ticket page shows an **Add to Google Wallet** button next to the Apple Wallet one. Operator setup: `plans/user-tasks/37-google-wallet-service-account.md` walks through the Google Pay & Wallet Console steps; the route 503s with a friendly message until the four `GOOGLE_WALLET_*` env vars are set
